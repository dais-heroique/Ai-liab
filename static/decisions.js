const conforvaOriginalRender=render;
const conforvaOriginalLoad=load;
...
async function load(){await conforvaOriginalLoad();if(location.hash.slice(1)==='decisions'){await loadConforvaDecisions();conforvaOriginalRender('decisions')}}
function show(view){if(view==='decisions'){location.hash='decisions';document.querySelectorAll('aside button[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view==='decisions'));closeMenu();loadConforvaDecisions().then(()=>decisions(document.getElementById('main')));return}conforvaOriginalShow(view)}
const conforvaOriginalShow=window.show;
function render(view){if(view==='decisions'){decisions(document.getElementById('main'));return}conforvaOriginalRender(view)}
window.render=render;window.show=show;window.load=load;window.decisions=decisions;window.openDecision=openDecision;
if(location.hash.slice(1)==='decisions')loadConforvaDecisions().then(()=>decisions(document.getElementById('main')));
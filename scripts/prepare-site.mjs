...
for(const [file,[path,title,description]] of Object.entries(pages)){
 ...
 s=s.replace(/<title>.*?<\/title>/i,`<title>${title}</title>`).replace(/<link[^>]+rel=["']canonical["'][^>]*>/gi,'').replace(/<meta[^>]+name=["']description["'][^>]*>/gi,'').replace(/<script type=["']application\/ld\+json["'][\s\S]*?<\/script>/gi,'');
